/* ================================================================
   ...
   ── HOW IT WORKS ──────────────────────────────────────────────
   Three EMA ribbon (5/13/34). Price pulls back to mid EMA (13)
   and a momentum candle fires = signal. Zero repainting.
   No blinking — redraw only happens on closed bars.
================================================================ */
...
class MyIndicator extends UserDefinedIndicator {
    ...
    onInit(data) {
        ...
        return {
            ...
            plots:     [],
            settingsFields: [
                {id:"fastLen",   caption:"Fast EMA period",               type:"int",   defaultValue:5,      min:2,   max:50},
                {id:"midLen",    caption:"Mid EMA period (pullback level)",type:"int",   defaultValue:13,     min:5,   max:100},
                {id:"slowLen",   caption:"Slow EMA period",               type:"int",   defaultValue:34,     min:10,  max:300},
                {id:"atrMult",   caption:"Min candle size ATR multiplier", type:"float", defaultValue:0.2,    min:0.1, max:3.0},
                {id:"minBars",   caption:"Min bars between signals",      type:"int",   defaultValue:1,      min:1,   max:50},
                ...
                {id:"colorBuy",  caption:"BUY signal color",              type:"color", defaultValue:"#00dcdc"},
                {id:"colorSell", caption:"SELL signal color",             type:"color", defaultValue:"#ff3333"},
    ...
    onCalculate(data, output) {
        ...
        var atrMult    = parseFloat(p.atrMult)  || 0.2;
        var minBars    = parseInt(p.minBars)    || 1;
        ...
        // This is what prevents blinking. removeAllDrawings() is
        // never called on a live tick — only on closed bars.
        if (data.currentBarUpdateOnly) {
            ...
            return; // ← skip redraw — no blink, no flash, no crash
        }

        // ── CLOSED BAR: full load + redraw ───────────────────────
        ...
        for (var i = 1; i <= scanLimit; i++) {
            ...
            var bullMomentum = close > open && (close - low)  / range >= 0.45 && close > fast;
            var bearMomentum = close < open && (high - close) / range >= 0.45 && close < fast;

            var signalType = null;
            if (showBuys  && bullTrend && bullMomentum && (touchedMidBull || nearMidBull)) signalType = "buy";
            if (!signalType && showSells && bearTrend && bearMomentum && (touchedMidBear || nearMidBear)) signalType = "sell";

            if (!signalType) { this.$signalCache[dateStr] = null; continue; }

            var isBuy = signalType === "buy";
            this.$signalCache[dateStr] = {
                dir: signalType, price: isBuy ? low : high,
                close: close, barIdx: i
            };
            lastSigBar = i;
            ...
            if (i === 1 && barDate !== this.$alertedDate) {
                ...
                this.$alertedClose = 0;
                this.$openSignal   = {
                    dir: signalType, entryPrice: close,
                    alert10:false, alert15:false, alert20:false, alert30:false
                };
                var title = (isBuy ? "▲ BUY" : "▼ SELL") + " — " + instrument;
                var msg   = "EMA pullback + momentum  |  " + timeframe + "  |  " + close.toFixed(2);
                if (alertsOn) {
    ...
    _arrow(date, price, isBuy, label, color) {
        return {
            ...
            iconColor:     color || (isBuy ? "#00dcdc" : "#ff3333"),
    ...
    _milestones(close, barDate, pipSize, alertsOn, telegramOn,
        ...
        fire(pp >= 10, "alert10",
            ...
            "Move stop to entry  |  Risk = ZERO  |  " + timeframe);
        fire(pp >= 15, "alert15",
            "⚡ +" + pp + " PIPS — Start Trailing | " + instrument,
            "Trail stop 10 pips behind price  |  " + timeframe);
        fire(pp >= 20, "alert20",
            "🚀 +" + pp + " PIPS — Close 50% | " + instrument,
            "Close half now  |  Trail the rest  |  " + timeframe);
        fire(pp >= 30, "alert30",
            "💰 +" + pp + " PIPS — Trail Tight | " + instrument,
            "Strong move  |  Trail tight  |  " + timeframe);
